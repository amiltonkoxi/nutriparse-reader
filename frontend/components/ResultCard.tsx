...
export function ResultCard({ data }: { data: ApiResponse }) {
  const { meta, allergens, nutrition, diagnostics } = data;
      ...
      <CardContent>
        <Tabs defaultValue="table" className="w-full">
          ...
          <TabsContent value="table" className="pt-4 space-y-6">
                  ...
                  {allergenEntries.map(([key, v]) => (
                    <Tooltip key={key}>
                      ...
                      <TooltipContent className="max-w-sm">
                        <p className="text-xs whitespace-pre-wrap">{v.evidence || "(no evidence)"}</p>
                      </TooltipContent>
                    </Tooltip>
                  ))}
                </div>
            ...
            <section>
              <h3 className="font-medium mb-2">Nutrition (per 100g when available)</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm border">
                  <tbody>
                    ...
                    <tr className="border-b"><td className="p-2 font-medium">Fat</td><td className="p-2">{nutrition.fat_g ?? "—"} g</td></tr>
                    ...
                    <tr className="border-b"><td className="p-2 font-medium">Salt</td><td className="p-2">
                      {nutrition.salt_g ?? (nutrition.sodium_g ? `${(nutrition.sodium_g * 2.54).toFixed(3)} g (calc from sodium)` : "—")}
                    </td></tr>
                  </tbody>
                </table>
              </div>
          ...
          <TabsContent value="json" className="pt-4">
            <ScrollArea className="h-80 rounded border p-3 bg-slate-50">
          ...
          <TabsContent value="preview" className="pt-4">
            {diagnostics?.raw_text_preview ? (
              <ScrollArea className="h-80 rounded border p-3 bg-slate-50">
                <pre className="text-xs whitespace-pre-wrap">{diagnostics.raw_text_preview}</pre>
              </ScrollArea>
            ) : (
